        ...
        <div className="glass-card p-5 lg:p-6 rounded-[2rem] lg:rounded-[2.5rem] border border-white/80 shadow-2xl shadow-teal-900/5">
          <div className="flex bg-slate-100/50 p-1.5 rounded-2xl mb-6 shadow-inner">
            <button
               ...
               className={`flex-1 py-3 text-[9px] lg:text-[10px] font-bold uppercase tracking-widest rounded-xl transition-all duration-300 ${
                 dataSource === 'history' 
          ...
          <div className="space-y-4 max-h-[500px] lg:max-h-[650px] overflow-y-auto pr-2 custom-scrollbar">
            {isLoading ? (
              <div className="py-20 text-center">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Sinkronisasi Data...</p>
              </div>
              ...
              Object.entries(
                ...
                <div key={monthName} className="mb-8 last:mb-0">
                  <div className="flex items-center justify-between mb-4 px-2">
                    <span className="text-[10px] font-bold text-slate-400 tracking-widest uppercase">{monthName}</span>
                    <span className="h-px bg-slate-100 flex-1 mx-4"></span>
                    <span className="text-[10px] font-bold text-teal-500 bg-teal-50 px-2.5 py-1 rounded-lg border border-teal-100">{groups.length}</span>
                  </div>
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-2">
                             <span className={`text-[9px] font-bold px-2 py-1 rounded-md uppercase tracking-widest ${selectedGroup?.key === group.key ? 'bg-teal-600 text-white' : 'bg-white text-slate-400 shadow-sm border border-slate-100'}`}>
                               {group.vendor}
                             </span>
                          </div>
                        ...
                        <div className={`text-[13px] font-semibold leading-snug line-clamp-2 ${selectedGroup?.key === group.key ? 'text-slate-900' : 'text-slate-600'}`}>
                          {group.items.map((i: any) => i.budgetDescription).join(', ')}
                        </div>
                        <div className="mt-4 flex items-center justify-between opacity-90 border-t border-slate-100 pt-3">
                           <span className="text-[11px] font-bold text-teal-600 font-sans tracking-tight">
                             {formatCurrency(group.totalAmount)}
                           </span>
                           <div className="flex items-center gap-2">
                              <FileText size={12} className="text-slate-300" />
                              <span className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">{group.items.length} Item</span>
                           </div>
                        </div>
                      </button>
                    ))}
      ...
      <div className="lg:col-span-8">
        {selectedGroup ? (
            ...
            <div className="bg-teal-50/20 backdrop-blur-md px-6 lg:px-12 py-8 lg:py-12 border-b border-slate-100/50 flex flex-col md:flex-row md:items-center justify-between gap-8">
              <div className="flex items-center gap-6">
                <div className="w-16 h-16 bg-white rounded-2xl lg:rounded-3xl shadow-xl shadow-teal-900/5 text-teal-600 flex items-center justify-center border border-teal-50/50 shrink-0">
                 ...
                 <div>
                    <h3 className="text-xl lg:text-2xl font-bold text-slate-800 tracking-tight leading-none mb-3">{selectedGroup.vendor}</h3>
                    <div className="flex flex-wrap items-center gap-2">
                       <span className="px-3 py-1.5 bg-white/80 rounded-lg text-[10px] font-bold text-slate-500 uppercase tracking-widest border border-slate-100 font-sans">{new Date(selectedGroup.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                       <span className="px-3 py-1.5 bg-teal-600 text-white rounded-lg text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-teal-500/20">{selectedGroup.items.length} Item Terdeteksi</span>
                    </div>
                 </div>
               </div>
               <button 
                 ...
                 className="group w-full md:w-auto flex items-center justify-center gap-3 px-8 py-4 btn-primary-glass border-none text-white rounded-2xl text-[11px] font-bold uppercase tracking-widest transition-all duration-500 shadow-2xl shadow-teal-500/30 disabled:opacity-50"
               >
            ...
            <div className="p-6 lg:p-12">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                <div className="btn-primary-glass p-6 lg:p-10 rounded-[2rem] lg:rounded-[3rem] text-white shadow-2xl shadow-teal-500/20 relative overflow-hidden group border-none">
                    ...
                    <div className="flex justify-between items-center mb-8 border-b border-white/20 pb-6">
                        <h4 className="text-[10px] font-bold uppercase tracking-[0.3em] text-teal-100 flex items-center gap-2 font-sans">
                            <Receipt size={14} /> Total Transaksi
                        </h4>
                        <span className="text-lg lg:text-2xl font-bold font-sans tracking-tight">
                            {formatCurrency(selectedGroup.totalAmount)}
                        </span>
                    </div>
                    <div className="space-y-3.5">
                      {selectedGroup.items.slice(0, 3).map((item: any, idx: number) => (
                        <div key={idx} className="flex justify-between items-center text-[11px] font-bold text-white/90">
                ...
                <div className="bg-slate-900 p-6 lg:p-10 rounded-[2rem] lg:rounded-[3rem] text-white shadow-2xl shadow-slate-900/40 relative overflow-hidden group">
                    ...
                    <h4 className="text-[10px] font-bold uppercase tracking-[0.3em] text-slate-400 mb-8 flex items-center gap-2 font-sans">
                         <Sparkles size={14} className="text-teal-400" /> Generator SPJ
                    </h4>
                    <div className="grid grid-cols-2 gap-4">
                       <button onClick={() => openPrintModal('kuitansi', selectedGroup)} className="p-5 lg:p-7 bg-slate-800/80 hover:bg-teal-600 border border-slate-700 hover:border-teal-500 rounded-2xl transition-all duration-500 group/btn flex flex-col items-center gap-2.5">
                          <Receipt size={24} className="text-teal-400 group-hover/btn:text-white" />
                          <span className="text-[9px] font-bold uppercase tracking-widest text-slate-400 group-hover/btn:text-white">Cetak Kuitansi</span>
                       </button>
                       <button onClick={() => openPrintModal('daftar_hadir', selectedGroup)} className="p-5 lg:p-7 bg-slate-800/80 hover:bg-slate-700 border border-slate-700 hover:border-slate-600 rounded-2xl transition-all duration-500 group/btn flex flex-col items-center gap-2.5">
                          ...
                          <span className="text-[9px] font-bold uppercase tracking-widest text-slate-400 group-hover/btn:text-white">Cetak Absensi</span>
                       </button>
                    </div>
                </div>
              </div>
              ...
              <div className="space-y-6 lg:space-y-8">
                <div className="flex items-center justify-between mb-8 px-2 overflow-hidden">
                    <p className="text-[9px] lg:text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] whitespace-nowrap">Audit Requirements</p>
                        ...
                        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8">
                          <div className="flex items-center gap-6">
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="text-sm lg:text-base font-bold text-slate-800 tracking-tight mb-1.5 whitespace-normal break-words leading-snug">{evidence}</div>
                              <div className="flex items-center gap-2">
                                {files.length > 0 ? ( 
                                  <span className="text-[9px] font-bold text-emerald-600 uppercase tracking-widest bg-emerald-100/50 px-3 py-1 rounded-lg border border-emerald-100">{files.length} Berkas Tersimpan</span> 
                                ) : ( 
                                  <span className="text-[9px] font-bold text-slate-400 bg-slate-100/80 px-3 py-1 rounded-lg border border-slate-200">Belum Ada Bukti</span> 
                                )}
                              </div>
                            </div>
                          </div>
                          
                          <label className={`relative overflow-hidden group/btn px-8 lg:px-12 py-5 lg:py-6 rounded-[2rem] text-[10px] font-bold uppercase tracking-widest cursor-pointer transition-all duration-700 w-full xl:w-auto text-center ${isUploading ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'btn-primary-glass border-none text-white hover:scale-105 active:scale-95'}`}>
          ...
          <div className="glass-card rounded-[3rem] lg:rounded-[4rem] border-2 border-dashed border-teal-200/50 flex flex-col items-center justify-center p-8 lg:p-20 text-center h-full min-h-[500px] lg:min-h-[650px] group transition-all duration-700 hover:bg-white/60 hover:border-teal-300">
            ...
            <h3 className="text-xl lg:text-3xl font-bold text-slate-800 mb-4 tracking-tighter leading-none">Mulai Audit <span className="text-teal-600">Digital</span></h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 w-full max-w-4xl">
              {[
                ...
                <div key={i} className="p-6 lg:p-8 rounded-[2rem] lg:rounded-[2.5rem] bg-white border border-slate-100 shadow-xl shadow-teal-900/5 hover:-translate-y-3 transition-all duration-700 group/card">
                  ...
                  <div className="text-[10px] text-teal-600 uppercase tracking-widest font-bold mb-1.5 font-sans">Tahap {s.step}</div>
                  <div className="text-sm lg:text-base font-bold text-slate-800 mb-2 leading-none">{s.title}</div>
