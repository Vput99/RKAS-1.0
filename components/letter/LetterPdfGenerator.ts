...
const drawKopSurat = (doc: any, data: LetterAgreement, profile: SchoolProfile | null) => {
  ...
  if (profile?.headerImage) {
    try {
      doc.addImage(profile.headerImage, 'PNG', margin + 5, 12, 22, 22);
    } catch (e) {
...
const drawPasal = (doc: any, no: string, judul: string, isi: string[], margin: number, pw: number, yRef: { y: number }) => {
  ...
  doc.setFontSize(11);
  doc.setFont('times', 'bold');
  doc.text(`Pasal ${no}`, pw / 2, yRef.y, { align: 'center' }); yRef.y += 6;
  doc.text(judul, pw / 2, yRef.y, { align: 'center' }); yRef.y += LINE + 2;
...
export const generateEkskulPDF = (data: LetterAgreement, profile: SchoolProfile | null) => {
  const doc = new jsPDF('p', 'mm', 'a4');
  const pw = doc.internal.pageSize.getWidth();
  const margin = 20;
  const LINE = 8.25;

  let y = drawKopSurat(doc, data, profile);

  doc.setFontSize(13);
  doc.setFont('times', 'bold');
  doc.text('SURAT PERJANJIAN KERJA (MOU)', pw / 2, y + 2, { align: 'center' });
  doc.setFontSize(11);
  doc.text('TENAGA PELAKSANA KEGIATAN EKSTRAKURIKULER', pw / 2, y + 8, { align: 'center' });
  doc.setFontSize(10);
  doc.text(`TAHUN PELAJARAN ${data.fiscal_year}`, pw / 2, y + 13, { align: 'center' });
  y += 22;

  doc.setFont('times', 'normal');
  doc.setFontSize(11);
  doc.text(`Nomor : ${data.letter_number || '...'}`, pw / 2, y, { align: 'center' });
  y += 12;
  ...
  doc.text(wrPembuka, margin, y, { lineHeightFactor: 1.5 });
  ...
  const drawPihak = (label: string, rows: [string, string][], catatan: string) => {
    ...
    rows.forEach(([k, v]) => {
      ...
      doc.text(wv, colVal, y, { lineHeightFactor: 1.5 });
      y += wv.length * 6;
    });
    y += 2;
    doc.setFont('times', 'normal');
    ...
    doc.setFont('times', 'bold');
    doc.text(catatan + '.', margin + doc.getTextWidth('Selanjutnya disebut sebagai '), y);
    y += LINE + 3;
  };
  ...
  doc.setFont('times', 'normal');
  doc.text(wrPenghubung, margin, y, { lineHeightFactor: 1.5 });
...
export const generateTukangPDF = (data: LetterAgreement, profile: SchoolProfile | null) => {
  const doc = new jsPDF('p', 'mm', 'a4');
  const pw = doc.internal.pageSize.getWidth();
  const margin = 20;
  const LINE = 8.25;
  ...
  doc.text('SURAT PERJANJIAN KERJA (MOU)', pw / 2, y + 2, { align: 'center' });
  doc.setFontSize(11);
  doc.text('PEKERJAAN REHABILITASI GEDUNG/BANGUNAN', pw / 2, y + 8, { align: 'center' });
  doc.setFontSize(10);
  doc.text(`TAHUN ANGGARAN ${data.fiscal_year}`, pw / 2, y + 13, { align: 'center' });
  y += 22;
